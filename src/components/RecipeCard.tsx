...
const RecipeCard = ({ recipe, matchPercentage, onClick, onAddToWeek, isInWeek }: RecipeCardProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Content — strict 8pt spacing, fixed min-height for alignment */}
      <div className="p-4 flex flex-col flex-1 min-h-[180px]">
        {/* Title — exactly 2 lines max */}
        <h3 className="font-display text-lg tracking-tight text-foreground leading-tight group-hover:text-primary transition-colors line-clamp-2 min-h-[48px]">
          {recipe.title}
        </h3>

        {/* Metadata row — fixed height */}
        <div className="h-6 flex items-center gap-4 text-sm text-muted-foreground mt-2">
          {totalTime && (
        ...
        <div className="h-6 flex items-center mt-2">
          {matchPercentage !== undefined && matchPercentage > 0 && (
            <div className="flex items-center gap-2 w-full">
              ...
              <span className="text-xs font-medium text-kitchen-herb whitespace-nowrap">
